...
function Cards({ title , notifievent , clubname , img , id} : { id: any; title: string; notifievent: string; clubname: string; img: string; }) {
  ...
  return (
    ...
    onClick={handleclick} key={id} >
      <div className="text-xl text-white uppercase">{title}</div>
      <div className='border-2 border-slate-500 p-4 bg-white bg-opacity-20 backdrop-blur-lg w-full h-10 line-clamp-1  drop-shadow-lg text-black text-xs z-20 flex justify-center items-center'>
        <span className='z-10'>
        {notifievent} 
        </span>
      </div>
